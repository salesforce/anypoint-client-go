...
For using other server than the one defined on index 0 set context value `ame_binding.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `ame_binding.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `ame_binding.ContextOperationServerIndices` and `ame_binding.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), ame_binding.ContextAccessToken, "BEARER_TOKEN_STRING")
