# Go API client for user_rolegroups
...
For using other server than the one defined on index 0 set context value `user_rolegroups.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `user_rolegroups.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `user_rolegroups.ContextOperationServerIndices` and `user_rolegroups.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), user_rolegroups.ContextAccessToken, "BEARER_TOKEN_STRING")
