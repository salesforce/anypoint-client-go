...
For using other server than the one defined on index 0 set context value `role.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `role.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `role.ContextOperationServerIndices` and `role.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), role.ContextAccessToken, "BEARER_TOKEN_STRING")
