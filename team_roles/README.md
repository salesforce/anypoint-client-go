...
For using other server than the one defined on index 0 set context value `team_roles.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `team_roles.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `team_roles.ContextOperationServerIndices` and `team_roles.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), team_roles.ContextAccessToken, "BEARER_TOKEN_STRING")
