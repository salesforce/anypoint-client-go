...
For using other server than the one defined on index 0 set context value `rm_alerts.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `rm_alerts.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `rm_alerts.ContextOperationServerIndices` and `rm_alerts.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), rm_alerts.ContextAccessToken, "BEARER_TOKEN_STRING")
