...
For using other server than the one defined on index 0 set context value `secretgroup_truststore.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `secretgroup_truststore.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `secretgroup_truststore.ContextOperationServerIndices` and `secretgroup_truststore.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), secretgroup_truststore.ContextAccessToken, "BEARER_TOKEN_STRING")
