...
For using other server than the one defined on index 0 set context value `secretgroup_keystore.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `secretgroup_keystore.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `secretgroup_keystore.ContextOperationServerIndices` and `secretgroup_keystore.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), secretgroup_keystore.ContextAccessToken, "BEARER_TOKEN_STRING")
